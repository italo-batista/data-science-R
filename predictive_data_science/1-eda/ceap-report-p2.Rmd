...
investiment.mn.filter.year = investiment.mn %>%
  filter(ano <= max.year & ano >= min.year)
...
data.ceap.by.year %>%
  ...
  geom_line(data=investiment.mn.filter.year, aes(x=ano, y=investimento)) +
  scale_x_continuous(trans='log2', breaks=c(min.year:max.year))
