...
get_plot_qst_3 <- function(yVar, title, yLab, yScaleUnity, yScaleRound) {
  ...
  plot = data.ceap.pb %>%
    ...
    scale_color_drsimonj(discrete = TRUE, palette = "cool") +
...
```{r echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.height=6, fig.width=8}
data.ceap.month.exceeded %>%
  ...
  coord_flip() +
  facet_wrap(~sgUF, ncol = 2, scales = "free") +
...
data.ceap.uf.airfare %>%
  ...
  labs(title="Federation unity that most spent with airfares",
       y="")
```
