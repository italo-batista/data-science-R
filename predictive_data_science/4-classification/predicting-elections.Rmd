...
```{r}
mFitControl = trainControl(method = "cv",
                           number = 5,
                           classProbs = TRUE,
                           summaryFunction = twoClassSummary)
...
```{r}
print_metrics(svmFit, validation_set)
```

#### No treino:

```{r}
print_metrics(svmFit, train)
```
...
write_output(svmFit, test, "svm_predictions.csv")
