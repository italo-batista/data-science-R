...
dados %>% 
    ...
    geom_point(alpha = .4) + 
    geom_jitter(width = .4) +
    geom_boxplot(alpha = 0.7, widht = 1.6)
```


No gráfico acima, a linha marrom identifica a mediana da avaliação para cada série. A partir deste gráfico, identificamos que Game of Thrones (GoT), Sense8 e Stranger Things competem entre as melhores, pois 50% dos episódios dessas séries são melhor avaliados que a maioria dos episódios de Black Mirror de The Simpsons.
...
Podemos também analisar a proporção de pessoas que deram avaliações mais altas.
