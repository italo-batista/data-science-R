...
Primeiro, vamos entender se comportam segundo essas duas variáveis juntas!
...
```
```{r}
filmes_scales = filmes %>% mutate(`BOX OFFICE` = log10(`BOX OFFICE`))
filmes_scale = filmes
filmes_scale$`BOX OFFICE` = log10(filmes$`BOX OFFICE`)
```
...
Percebemos que quando há cinco ou mais grupos, alguns pontos de um grupo acabam se afastando de seu grupo e ficando mais próximo de outro grupo. Vamos plotar um gráfico silhueta para identificar a qualidade do agrupamento.
...
plot(
  ...
  main="Gráfico 09 - Diagnóstico do agrupamento",
  yaxt='n', ann=FALSE)
```
...
```{r warning=FALSE}
distancias_scale = agrupamento_scale %>% 
    column_to_rownames("TITLE") %>%
    select(RATING, `BOX OFFICE`) %>% 
    #mutate(`BOX OFFICE` = log10(`BOX OFFICE`)) %>% 
    #mutate_all(funs(scale)) %>% 
...
plot(
  silhouette(cutree(agrupamento_h_2d, k = 6), distancias_scale), 
  col=c("#CDB380", "#036564", "#EB6841", "#EDC951", "darkred", "pink"),
  main="Gráfico 09 - Diagnóstico do agrupamento",
  yaxt='n', ann=FALSE)
```
